...
const HomeVisualizationApp = () => {
  ...
  const handleCanvasMouseDown = (e) => {
    ...
    if (mode === 'point') {
      ...
      setBoxStart({ x, y });
      setBoxEnd({ x, y }); // Initialize end point
    }
  };
  ...
  const handleCanvasClick = (e) => {
    ...
    setPoints([...points, { x, y, type: 'foreground' }]);
  };
  ...
  const handleCanvasRightClick = (e) => {
    ...
    setPoints([...points, { x, y, type: 'background' }]);
  };
  ...
  const processImage = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5001/api/edit', {
        method: 'POST',
        headers: {
